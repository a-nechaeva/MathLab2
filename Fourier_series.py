...
class Fourier(Scene):
    def construct(self):
        ...
        txt_message = Text(message, font_size=30)
        txt_message.set_color(COLOR_MESSAGE)

        math_func = MathTex(tex_func, font_size=30)
        math_func.set_color(COLOR_FUNC)

        math_tilde = MathTex(tex_sim_or_equals, font_size=40)
        math_tilde.set_color(COLOR_TILDE)

        math_series = MathTex(tex_series, font_size=30)
      ...
      #  txt_periodic = Text("periodic", font_size=22, color=COLOR_PERIODIC).next_to(math_func, DOWN, buff=0.2)
        ...
        self.play(ReplacementTransform(txt_message, math_func),Create(graph_func))
    ...
    def create_axes(self):
        ...
        labels = self.axes.get_axis_labels(x_label="x", y_label="y").set_color(WHITE)
