...
def series(x, num_terms):
    ...
    for n in range(num_terms, 0, -1):
        first_part = 4 * np.sin(0.5 * np.pi * n) / (np.pi * n) ** 2
        second_part = 2 * np.cos(0.5 * np.pi * n) / (np.pi * n)
        sum_part = first_part + second_part
        mult_part = sum_part * np.sin(0.5 * np.pi * n * x)
        sum_terms += mult_part
...
tex_series = r"""
\sum_{n=1}^{\infty}
    \left( \frac{4 \sin \frac{\pi n}{2}}{(\pi n)^2} +
    \frac{2 \cos \frac{\pi n}{2}}{\pi n} \right) \sin \frac{\pi n x}{2}
"""
